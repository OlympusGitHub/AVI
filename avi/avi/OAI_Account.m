...
- (void) buildAccountObjects  { 
    ...
    [btnCloseX addTarget:self action:@selector(toggleWin:) forControlEvents:UIControlEventTouchUpInside];
        [btnCloseX setBackgroundColor:[UIColor clearColor]];
        [self addSubview:btnCloseX];
   
}

- (void) toggleWin:(UIButton *)btnClose {
    
    CGRect myFrame = CGRectMake(100, 0.0-self.frame.size.height, self.frame.size.width, self.frame.size.height);
